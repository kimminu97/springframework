...
<body>
	...
	<form action="gallery" method="post" enctype="multipart/form-data">
		...
		이미지 파일을 선택하세요 (최대 3개)<br> <input type="file" name="pics"
			accept="image/*"><br> <input type="file" name="pics"
			accept="image/*"><br> <input type="file" name="pics"
			accept="image/*"><br>
		<button>전송</button>
	</form>
	...
	url이 필요합니다. c:/upload서버 폴더에 파일을 저장했으나 클라이언트에서는 폴더로 
	접근하는 것이 아니고 url(예, /upload)로 접근해야합니다. server.xml에서 설정합니다.-->
	<c:forEach items="${list }" var="vo"><!-- 업로드한 3개의 파일명을 리스트로 에트리뷰트에서 가져오기 -->
		<div>
